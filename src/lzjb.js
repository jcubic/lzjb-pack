...
(async function() {
    const res = await fetch('https://cdn.jsdelivr.net/npm/sysend@1.7.0');
    const input = new Uint8Array(await res.arrayBuffer());
    const output = unpack(pack(input));
    
    const decoder = new TextDecoder('utf-8', {fatal: true});
    out.innerHTML = decoder.decode(output);
})();